...
def load_content_module():
    path = COMP / "content.py"
    spec = importlib.util.spec_from_file_location("matrix_extended_content_v052", path)
...
def test_send_exposes_native_home_assistant_media_picker() -> None:
...
def test_media_picker_output_is_normalized_for_existing_media_resolver() -> None:
    ...
    result = content.media_picker_to_media_item(
        {
...
def test_service_localizations_exist_in_english_and_russian() -> None:
    ...
    assert ru["services"]["send"]["fields"]["media_picker"]["name"] == "Медиа — выбрать"
